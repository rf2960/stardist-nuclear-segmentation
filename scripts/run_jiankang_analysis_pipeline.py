...
SLIDE_PATH = ROOT / "Jiankang TMA.svs"
OUT_DIR = ROOT / "results_jiankang_analysis"
...
def segment_slide(slide, model, cores, source_mpp):
    ...
    for core in cores:
        core_num = core["core"]
        print(f"Processing Jiankang core {core_num}/7", flush=True)
        cx = core["x_full"]
        cy = core["y_full"]
        radius_model = core["radius_full"] / model_scale
        margin = BOUNDARY_MARGIN_MODEL.get(core_num, 64)
    ...
    print(f"Final Jiankang nuclei: {len(df)}", flush=True)
...
def write_summary_tables(payload):
    ...
    summary_df.to_csv(OUT_DIR / "analysis_summary_jiankang.csv", index=False)
    summary_df[["core", "cells"]].to_csv(OUT_DIR / "core_counts_jiankang.csv", index=False)
...
<title>Jiankang StarDist Nuclear Viewer</title>
<style>
...
.modal {{ position:fixed; inset:0; background:rgba(15,23,42,.72); display:none; align-items:center; justify-content:center; z-index:20; padding:12px; }}
.modal.open {{ display:flex; }}
.modal-inner {{ background:#fff; border-radius:8px; width:min(96vw, 1600px); height:94vh; overflow:hidden; padding:12px; box-shadow:0 20px 80px rgba(0,0,0,.35); display:grid; grid-template-rows:auto minmax(0,1fr); }}
.modal-bar {{ display:flex; justify-content:space-between; align-items:center; gap:12px; margin-bottom:8px; color:var(--muted); flex-wrap:wrap; }}
.modal-tools {{ display:flex; align-items:center; gap:12px; flex-wrap:wrap; }}
.modal-close {{ border:1px solid var(--line); background:#fff; border-radius:7px; padding:6px 10px; cursor:pointer; }}
.modal-content {{ min-height:0; display:flex; align-items:center; justify-content:center; background:#f8fafc; border:1px solid var(--line); border-radius:6px; overflow:hidden; }}
.modal-image {{ max-width:100%; max-height:100%; }}
.modal-image img {{ max-width:calc(96vw - 52px); max-height:calc(94vh - 92px); width:auto; height:auto; object-fit:contain; }}
@media (max-width:900px) {{ main {{ grid-template-columns:1fr; }} aside {{ border-right:0; border-bottom:1px solid var(--line); }} }}
</style>
</head>
<body>
<header>
  <h1>Jiankang StarDist Nuclear Viewer</h1>
...
<div class="modal" id="modal"><div class="modal-inner"><div class="modal-bar"><span id="modalTitle"></span><div class="modal-tools"><label><input id="modalSegToggle" type="checkbox" checked> Boundary</label><label>Opacity <input id="modalAlpha" type="range" min="0" max="100" value="96"></label><button class="modal-close" id="modalClose">Close</button></div></div><div id="modalContent" class="modal-content"></div></div></div>
<script>
...
function showPatch(p) {{ document.getElementById("modalTitle").textContent=`Core ${{coreKeys.indexOf(selected)+1}} - row ${{p.row}}, col ${{p.col}} - ${{p.n_cells}} nuclei`; document.getElementById("modalContent").innerHTML=`<div class="image-wrap modal-image"><img id="modalImg" src="data:image/jpeg;base64,${{p.img_hi}}" width="${{p.hi_width}}" height="${{p.hi_height}}"><canvas class="overlay"></canvas></div>`; const toggle=document.getElementById("modalSegToggle"); const alpha=document.getElementById("modalAlpha"); toggle.checked=showSegmentation; alpha.value=Math.round(boundaryAlpha*100); toggle.onchange=e=>{{showSegmentation=e.target.checked;repaintModal(p);renderPatches();}}; alpha.oninput=e=>{{boundaryAlpha=Number(e.target.value)/100;repaintModal(p);renderPatches();}}; document.getElementById("modal").classList.add("open"); repaintModal(p); }}
...
def main():
    ...
    print("Detected Jiankang cores:", [(round(c["x_thumb"], 1), round(c["y_thumb"], 1)) for c in cores], flush=True)

    model = StarDist2D.from_pretrained("2D_versatile_he")
    model_scale = TARGET_MPP / source_mpp
    read_size = int(round(PATCH_SIZE * model_scale))
    csv_path = OUT_DIR / "detected_cells_jiankang.csv"
    ...
    (OUT_DIR / "analysis_data_jiankang.json").write_text(json.dumps(payload), encoding="utf-8")
    write_summary_tables(payload)
    write_html(payload, OUT_DIR / "jiankang_stardist_analysis.html")
