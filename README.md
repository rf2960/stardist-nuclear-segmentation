...
Download the current viewers from the Git LFS files in this repository:

- [`results_1path_analysis/1path_stardist_analysis.html`](results_1path_analysis/1path_stardist_analysis.html)
- [`results_jiankang_analysis/jiankang_stardist_analysis.html`](results_jiankang_analysis/jiankang_stardist_analysis.html)

Because the viewers are self-contained files larger than GitHub's normal file-size limit, they are stored with Git LFS. Use the GitHub download button or clone the repository with Git LFS enabled to open them locally.
...
## Detected-Cell CSV Files

The full per-cell detection tables are generated by the analysis pipelines:

- `results_1path_analysis/detected_cells_1path.csv`
- `results_jiankang_analysis/detected_cells_jiankang.csv`

These files are tracked with Git LFS because they can be tens of MB and contain one row per accepted nucleus. They are the right files to use when downstream analysis needs exact cell locations or segmentation boundaries.

Each row represents one final deduplicated nucleus. The centroid columns reproduce the old small-circle overlays, while the boundary columns preserve the actual final segmentation outline.

| Column group | Columns | Meaning |
| --- | --- | --- |
| Identity | `core`, `cell_id` | Core number and per-core cell index. |
| Centroid | `x_full`, `y_full` | Cell center in original level-0 whole-slide pixels. |
| Model-space centroid | `x_model`, `y_model` | Cell center relative to the detected core center after scale normalization. |
| Area | `area_model_px` | Final instance-mask area in model-scale pixels. |
| Source patch | `source_patch_row`, `source_patch_col`, `source_patch_x_model`, `source_patch_y_model` | Overlapping inference tile where the cell was originally detected. |
| Assigned grid patch | `grid_patch_row`, `grid_patch_col`, `grid_patch_x_model`, `grid_patch_y_model` | Non-overlapping 512 x 512 grid patch assigned to the final cell for reproducible patch-level analysis. |
| Boundary summary | `boundary_n_points`, `boundary_area_model_px`, `boundary_perimeter_model_px`, `boundary_bbox_*_model` | Size and bounding-box measurements for the saved boundary polygon. |
| Boundary coordinates | `boundary_model_json` | Cell boundary polygon in core-relative model coordinates. |
| Boundary coordinates | `boundary_full_json` | Cell boundary polygon in original level-0 slide coordinates. Use this to fully reconstruct boundaries on the `.svs` slide. |
| Boundary coordinates | `boundary_grid_patch_json` | Cell boundary polygon relative to its assigned non-overlapping grid patch. |

Boundary coordinate columns are JSON arrays in the form `[[x1,y1],[x2,y2],...]`. The current pipelines export boundaries from the final non-overlapping label masks rather than the raw StarDist star-convex polygon approximation.
...
The comparison plots below summarize the final deduplicated, non-overlapping outputs for both candidate slides.

![Core count comparison](docs/figures/core_counts_comparison.png)

![Nuclear density comparison](docs/figures/density_comparison.png)
...
results_1path_analysis/
  ...
  detected_cells_1path.csv            # Git LFS full per-cell boundary table
  slide_comparison.csv

results_jiankang_analysis/
  jiankang_stardist_analysis.html     # Git LFS self-contained viewer
  analysis_summary_jiankang.csv
  core_counts_jiankang.csv
  detected_cells_jiankang.csv         # Git LFS full per-cell boundary table
  slide_comparison.csv

results_comparison.csv
```

Large raw slides and generated JSON payloads are intentionally ignored by Git. The mentor-facing detected-cell CSVs and self-contained HTML viewers are tracked with Git LFS.
